...
| `full-trader-example` | `examples/full-trader-example.ts`   | Reference bot flow: private streams via callbacks **and** the `orderUpdates()` async iterator, `MarketDataClient` on a second port, place / modify / cancel / mass-quote / batch-cancel, queue drain |
